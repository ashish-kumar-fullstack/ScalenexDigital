...
export async function deliverContact(input: unknown): Promise<ContactResult> {
  ...
  if (
    ...
    return {
      ok: false,
      message:
        "The contact form is temporarily unavailable. Please email scalenexdigital@gmail.com directly.",
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      message:
        "We couldn’t submit your enquiry right now. Please try again later or email us directly.",
    };
  }
  try {
    ...
    const info = await transporter.sendMail({
      ...
      text: [
        ...
        `Interested service: ${v.service}`,
        "",
        "Message:",
        v.message,
        "",
        "Customer agreed to be contacted about this enquiry.",
      ].join("\n"),
    });
    if (
      !info.accepted?.some(
    ...
    return {
      ok: true,
      message:
        "Thank you! Your enquiry has been sent. Our team will get back to you by email.",
    };
  } catch {
    return {
      ok: false,
      message:
        "Your enquiry wasn’t sent. Please try again later or email scalenexdigital@gmail.com directly.",
    };
  }
}
